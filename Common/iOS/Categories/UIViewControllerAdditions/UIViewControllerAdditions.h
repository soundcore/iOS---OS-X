/******************************************************************************

File:			UIViewControllerAdditions.h

Date:			9/13/10

Version:		1.1

Authors:		soundcore

				Copyright 2009 soundcore
				All rights reserved worldwide.

Notes:			Header for UIViewControllerAdditions.m

				Set tab width and indent width both to 4 In XCode's
				Text Editing Preferences.

WARNING:		UNDER NO CIRCUMSTANCES MAY THIS SOURCE CODE
				BE REDISTRIBUTED WITHOUT EXPRESS WRITTEN PERMISSION
				OF soundcore.

				ANY SUCH DISTRIBUTION CARRIES SEVERE CRIMINAL AND CIVIL
				PENALTIES AND IS A VIOLATION OF INTERNATIONAL
				COPYRIGHT LAW.

				VIOLATORS WILL BE PROSECUTED TO THE
				FULL EXTENT OF INTERNATIONAL LAW.

Dependencies:	XCode 3.2
				iPhone SDK 4.0+
Changes:		

9/13/10			MCA - Initial version.

******************************************************************************/

#import <UIKit/UIKit.h>

///////////
// Types
///////////

// Tweet result

typedef struct TweetResult
{
    BOOL	canTweet;
	BOOL	didTweet;
} TweetResult, *TweetResultPtr;

///////////
// Classes
///////////

@interface UIViewController ( UIViewControllerAdditions )

- (BOOL)viewControllerIsValidAndHasView;

// Core Animation

- (CATransition*)makeCannedCATransitionWithDuration:(NSUInteger)duration
					timingFucntionName:(NSString*)tfName
					type:(NSString*)tranType
					subtype:(NSString*)tranSubType
					removeOnCompletion:(BOOL)removeIt;

// Twitter

- (TweetResult)tweetMeSilly:(NSString*)initalTextKey andOptionalURL:(NSURL*)url;

// Utils

- (BOOL)osVersionIsAtLeast:(NSString*)desiredVersionString;

@end
